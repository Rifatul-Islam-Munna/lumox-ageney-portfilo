...
export default async function DynamicServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#f7f7f4] text-black">
      <section
        className="relative min-h-[500px] overflow-hidden bg-black text-white md:min-h-[600px]"
        style={{
          backgroundImage: `linear-gradient(90deg, rgba(0,0,0,.94), rgba(0,0,0,.76) 54%, rgba(0,0,0,.62)), url(${service.heroImage})`,
          backgroundPosition: "center",
          backgroundSize: "cover",
        }}
      >
        <SiteHeader />
        <div className="relative z-10 mx-auto flex max-w-[1400px] flex-col px-6 pt-28 sm:px-10 md:pt-40">
          <div className="max-w-[840px]">
            <p className="image-hero-kicker text-[13px] font-black uppercase tracking-[0.18em]">Dynamic service</p>
            <h1 className="image-hero-title mt-4 text-[46px] font-extrabold leading-none md:text-[78px]">
              {service.title}
            </h1>
            <p className="image-hero-body mt-7 max-w-[720px] text-[17px] font-semibold leading-[1.65]">{service.excerpt}</p>
      ...
      <section className="px-5 py-14 sm:px-10 md:py-20">
        <div className="mx-auto grid max-w-[1400px] gap-8 xl:grid-cols-[minmax(0,1fr)_360px]">
          <div>
            {visible.overview ? (
              <section className="grid overflow-hidden rounded-[8px] bg-white shadow-[0_18px_60px_rgba(0,0,0,0.07)] ring-1 ring-black/10 lg:grid-cols-[45%_55%]">
                <img
                ...
                <div className="p-7 sm:p-9 lg:p-11">
                  ...
                  <h2 className="mt-4 text-[32px] font-black leading-tight text-[#050505] md:text-[44px]">
            ...
            <div className="mt-8 grid gap-5 md:grid-cols-2">
              <article className="rounded-[8px] bg-white p-8 ring-1 ring-black/10 transition hover:-translate-y-0.5 hover:shadow-lg">
              ...
              <article className="rounded-[8px] bg-[#050505] p-8 text-white ring-1 ring-black/10 transition hover:-translate-y-0.5 hover:shadow-lg">
                <div className="grid h-14 w-14 place-items-center rounded-[6px] bg-[#ffd018]/18 text-[#967400]">
            ...
            {visible.process ? (
              <section className="mt-10 rounded-[8px] bg-[#050505] p-7 text-white sm:p-9">
            ...
            {(service.extraSections ?? []).filter((section) => section.enabled).map((section, index) => {
              ...
              return (
                <section className="mt-10 grid overflow-hidden rounded-[8px] bg-white ring-1 ring-black/10 lg:grid-cols-2" key={`${section.title}-${index}`}>
                  {section.image ? (
                    <img
                  ...
                  <div className={["p-7 sm:p-9", imageLeft ? "lg:order-2" : "lg:order-1"].join(" ")}>
            ...
            <section className="mt-14">
              ...
              <div className="mt-8 space-y-4">
                {service.faqs.map((faq, index) => (
                  <details className="group rounded-[8px] bg-white p-6 ring-1 ring-black/10" key={faq.question} open={index === 0}>
            ...
            <section className="rounded-[8px] bg-white p-7 ring-1 ring-black/10">
              ...
              <div className="mt-7 space-y-3">
                {visibleServices.map((item) => (
                  <Link
                    className={[
                      "flex items-center gap-3 rounded-[5px] border border-black/10 px-4 py-4 text-[14px] font-black uppercase transition hover:border-[#ffd018] hover:bg-[#ffd018]/15",
