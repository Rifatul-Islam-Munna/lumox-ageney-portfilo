...
import { SeoCmsForm } from "@/components/admin/seo-cms-form";
...
const nav = [
  ...
  { title: "SEO", href: "/admin/seo", icon: SearchIcon, active: true },
];

export default async function AdminSeoPage() {
  if (!(await isAdminAuthed())) redirect("/admin/login");

  let content = defaultSeoContent;
  try {
    content = (await getSeoContent()) ?? defaultSeoContent;
  } catch {
    content = defaultSeoContent;
  }
      ...
      <SidebarInset>
        <header className="sticky top-0 flex h-14 shrink-0 items-center gap-3 border-b bg-background px-4">
          ...
          <div className="min-w-0">
            <p className="truncate text-sm font-medium">SEO settings</p>
            <p className="truncate text-xs text-muted-foreground">Page metadata, social cards, robots, image alt, structured data.</p>
          </div>
        </header>
        <div className="p-4 md:p-6">
          <SeoCmsForm initialContent={content} />
        </div>
      </SidebarInset>
      <Toaster />
    </SidebarProvider>
